function JT(id,w,h,fps,setupName,updateName,objName,fullScreenBtn,compatibility){
    //constructor
    ...
    this.init=function(id,w,h,fps,setupName,updateName,objName,fullScreenBtn,compatibility){
		...
		if(compatibility!=undefined){
			if(typeof fullScreenBtn=="number" && typeof compatibility=="boolean"){
				fullScreenBtn=compatibility;
			}
		}
    ...
    this.canvas={
		...
		lastRatioW:undefined,
		lastRatioH:undefined,
		ratioFullscreen:false,
		keepRatio:true,
			...
			if(bool!=undefined){
				if(!fake){
					...
					if(bool){
						...
						this.context.drawing.cam[0].pos.w=this.lastRatioW;
    ...
    this.loop={
        ...
        mainLoop:function(){
            ...
            if(this.loaded==false){
                ...
                var load=true;
				if(!this.ios){
					this.context.assets.loadedIOS=true;
					for(var soun in this.context.assets.sounds){
						this.loadCounterMax++;
						...
						if(this.context.assets.sounds.hasOwnProperty(soun)){
							...
							if(sound!=undefined){
								if(sound.readyState<4){
									...
									this.context.assets.sounds[soun].muted=false;
									this.loadCounter++;
								}
							}
						}
    ...
    this.assets={
        ...
        sound:function(src,name,repeat,volume){
			...
			if(this.loop.ios){
				this.sounds[name].autoplay=true;
			}
            var context=this;
            if(repeat!=undefined){
                if(repeat==true){
    ...
    this.drawing={
        ...
        cam:[
			{x:0,y:0,w:undefined,h:undefined,r:0,active:true,pos:{x:0,y:0,w:undefined,h:undefined,cam:false}},
			{x:0,y:0,w:undefined,h:undefined,r:0,active:true,pos:{x:0,y:0,w:undefined,h:undefined,cam:false}},
			{x:0,y:0,w:undefined,h:undefined,r:0,active:true,pos:{x:0,y:0,w:undefined,h:undefined,cam:false}},
			{x:0,y:0,w:undefined,h:undefined,r:0,active:true,pos:{x:0,y:0,w:undefined,h:undefined,cam:false}},
			{x:0,y:0,w:undefined,h:undefined,r:0,active:true,pos:{x:0,y:0,w:undefined,h:undefined,cam:false}},
      ...
      camH=camH/posH;
        ...
        circle: function(x,y,d,color) {
			if(typeof(x)=="object"){
				...
				this.ctx.beginPath();
				this.fill("circle",x+d/2,y+d/2,d/2,d/2);
				if(color!=undefined){this.color(color,"fill");}
				this.ctx.fill();
			}
        },
            ...
            if(this.assets.images[name]!=undefined){
                if(image.visible==false){
                    ...
                    if(sX!=undefined){
						...
						this.ctx.drawImage(img,sX,sY,sW,sH,(x*camW)-(camX*camW)+posX,(y*camH)-(camY*camH)+posY,w*camW,h*camH);
                    }else{
						...
						this.ctx.drawImage(img,(x*camW)-(camX*camW)+posX,(y*camH)-(camY*camH)+posY,w*camW,h*camH);
                    }
                    this.ctx.restore();
                }
            }
	...
	this.touch={
        ...
        x:function(cam,touch){
            ...
            if(touch==undefined){touch=this.touches.length-1;}
    ...
    this.createEventListeners=function(context) {
			...
			if(document.activeElement.id==context.canvas.id){
				...
				/*if(context.canvas.fullscreen()){
					...
					if(ratioWindow>ratioGame){
						...
						var finalX=(evt.clientX-barW)*(w/actualW)

						context.mouse.x=finalX;
						context.mouse.cX = (finalX+(camX*camW)-posX)/camW;
					}else{
						...
						var finalY=(evt.clientY-barW)*(h/actualH)
		...
		context.canvas.src.addEventListener("click", function(evt) {
			if(!context.assets.loadedIOS){
				for(var i in context.assets.sounds){
					context.assets.sounds[i].play();
					
				}
				
				for(var i in context.assets.sounds){
					context.assets.sounds[i].pause();
					context.assets.sounds[i].muted=false;
					
				}
	
				context.assets.loadedIOS=true;
			}
		});
    ...
    //start
    this.init(id,w,h,fps,setupName,updateName,objName,fullScreenBtn,compatibility)
    ...
    this.fullscreen=function(bool,ratio){
        return this.canvas.fullscreen(bool,ratio);
    }
	...
	this.xToCam=function(x,num){
		return this.drawing.xCam(x,num);
	}

	this.yToCam=function(y,num){
		return this.drawing.yCam(y,num);
	}

	this.camToX=function(x,num){
		return this.drawing.camX(x,num);
	}

	this.camToY=function(y,num){
  ...
  this.clips=function(shapes){
	...
	this.mouseRightClick=function(bool){
		if(bool!=undefined){this.mouse.preventRight=bool;}
		return this.mouse.preventRight;
	}
	
	this.mRightClick=function(bool){
		if(bool!=undefined){this.mouse.preventRight=bool;}
		return this.mouse.preventRight;
	}
	...
	this.check=function(x,y,w,h,cam,btnFingers){
		if(!this.mobile.isAny()){
			if(typeof x=="object"){
				return this.mouse.check(x.x,x.y,x.w,x.h,false,x.cam,x.btn);
			}else{
				return this.mouse.check(x,y,w,h,false,cam,btnFingers);
			}
		}else{
			if(typeof x=="object"){
				return this.touch.check(x.x,x.y,x.w,x.h,false,x.cam,x.fingers);
			}else{
				return this.touch.check(x,y,w,h,false,cam,btnFingers);
			}
		}
	}
	
	this.press=function(x,y,w,h,cam,btnFingers){
		if(!this.mobile.isAny()){
			if(typeof x=="object"){
				return this.mouse.check(x.x,x.y,x.w,x.h,true,x.cam,x.btn);
			}else{
				return this.mouse.check(x,y,w,h,true,cam,btnFingers);
			}
		}else{
			if(typeof x=="object"){
				return this.touch.check(x.x,x.y,x.w,x.h,true,x.cam,x.fingers);
			}else{
				return this.touch.check(x,y,w,h,true,cam,btnFingers);
			}
		}
	}
	
	this.down=function(btn){
		if(!this.mobile.isAny()){
			if(btn==undefined){btn=0;}
			return this.mouse.down[btn];
		}else{
